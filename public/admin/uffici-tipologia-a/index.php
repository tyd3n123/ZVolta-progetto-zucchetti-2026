...
// ── Slot occupati per le scrivanie ────────────────────
$allIds = array_column($desks, 'id');
...
$stmt = $conn->prepare(
    "SELECT p.id_prenotazione, p.data_inizio, p.data_fine, a.codice_asset, a.id_asset
     ...
     WHERE p.id_utente = ? AND a.id_tipologia = 1 AND p.data_fine >= NOW()
...
<title>Scrivanie Tipologia A | Northstar</title>
...
<header class="header">
  <div class="header-left">
    ...
    <nav class="header-breadcrumb">
      ...
      <span class="bc-current">Scrivanie Tipologia A</span>
    </nav>
  </div>
...
<div class="uf-page">
  ...
  <div class="uf-title-row">
    <div>
      <h2 class="uf-page-title">Scrivanie Tipologia A</h2>
...
function buildLayout(W, H) {
    // Area scrivanie — centrata verticalmente nel canvas
    const DW   = W * 0.067;  // larghezza scrivania
    const DH   = H * 0.155;  // altezza scrivania
    const DG   = W * 0.006;  // gap tra scrivanie

    // Pod A (sinistra) e Pod B (destra)
    const PAX  = W * 0.030;
    const PBX  = W * 0.620;

    // Corridoio orizzontale in cima
    const NCY  = H * 0.060;
    const NCH  = H * 0.060;

    // Scrivanie partono dopo il corridoio nord
    const DAY  = NCY + NCH;
    const R1Y  = DAY;
    const R2Y  = DAY + DH + H * 0.055;

    // Corridoio verticale tra i pod (usato per il riempimento zona sud)
    const RSY  = R2Y + DH;

    return { W, H, DW, DH, DG, PAX, PBX, NCY, NCH, DAY, R1Y, R2Y, RSY };
}
...
function drawCorridors(W, H, l) {
    ...
    // Corridoio orizzontale nord
    ctx.fillRect(0, l.NCY, W, l.NCH);
    // Corridoio verticale centrale (tra pod A e pod B)
    const cX = l.PAX + 5 * (l.DW + l.DG);
    const cW = l.PBX - cX;
    ctx.fillRect(cX, l.DAY, cW, H - l.DAY);
    // Zona sud (sotto scrivanie)
    ctx.fillRect(0, l.RSY, W, H - l.RSY);
    // Bordi tratteggiati corridoio nord
...
function drawEntrance(W, H) {
    const ex = W / 2 - 36;
    // Cancella il muro per l'apertura
    ctx.fillStyle = C.corridorBg;
...
function drawDeskPods(W, H, l) {
    // Pod A: indici 0-9 | Pod B: indici 10-19
    const pods = [
        { start: 0,  startX: l.PAX },
        { start: 10, startX: l.PBX },
    ];
    for (const pod of pods) {
        for (let i = 0; i < 5; i++) {
            ...
            drawOneDesk(dx, l.R1Y, l.DW, l.DH, d, 'south');
            hitboxes.push({ x: dx, y: l.R1Y, w: l.DW, h: l.DH, item: d });
        }
        for (let i = 0; i < 5; i++) {
            const d = desksData[pod.start + 5 + i];
            if (!d) continue;
            const dx = pod.startX + i * (l.DW + l.DG);
            drawOneDesk(dx, l.R2Y, l.DW, l.DH, d, 'north');
            hitboxes.push({ x: dx, y: l.R2Y, w: l.DW, h: l.DH, item: d });
        }
        // Bordo pod tratteggiato
        const pw = 5 * (l.DW + l.DG) - l.DG;
        const ph = l.DH + (l.R2Y - l.R1Y - l.DH) + l.DH;
        ctx.strokeStyle = 'rgba(180,160,130,0.45)';
        ctx.lineWidth = 1;
        ctx.setLineDash([4, 3]);
        ctx.strokeRect(pod.startX - 4, l.R1Y - 4, pw + 8, ph + 8);
        ctx.setLineDash([]);
    }
}
...
function drawOneDesk(x, y, w, h, desk, facing) {
    ...
    // Codice
    const code = desk.name.replace('TIPO-A-', 'A-');
    ctx.fillStyle = isSel ? '#4c1d95' : C.textDark;
    ctx.font = 'bold ' + Math.max(8, Math.min(10, Math.floor(w/7))) + 'px system-ui';
...
function openPanel(item) {
    ...
    document.getElementById('panel-info-grid').innerHTML =
        `<div class="uf-info-tile"><div class="uf-tile-lbl">Tipologia</div><div class="uf-tile-val">Scrivania A</div></div>
...
// Riapri pannello dopo POST (prenotazione appena inviata)
if (reopenId) {
