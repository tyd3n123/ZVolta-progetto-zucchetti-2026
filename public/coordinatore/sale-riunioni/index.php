...
$stmt = $conn->prepare(
    "SELECT p.id_prenotazione, p.data_inizio, p.data_fine, a.codice_asset, a.id_asset
     ...
     WHERE p.id_utente = ? AND a.id_tipologia = 3 AND p.data_fine >= NOW()
...
$result = $conn->query(
    "SELECT p.id_asset, p.data_inizio, p.data_fine
     FROM prenotazioni p JOIN asset a ON p.id_asset = a.id_asset
     WHERE a.id_tipologia = 3 AND p.data_fine >= NOW()
     ORDER BY p.data_inizio ASC"
);
if ($result) {
    while ($row = $result->fetch_assoc()) {
        $occupiedSlotsByAsset[$row['id_asset']][] = ['inizio' => $row['data_inizio'], 'fine' => $row['data_fine']];
    }
}
...
<head>
    ...
    <link rel="stylesheet" href="../../admin/sale-riunioni/sale-riunioni.css">
</head>
<body>
...
<div class="sr-page">
        ...
        <div class="sr-map-zone" id="map-zone">
            ...
            <div class="sr-map-header">
                ...
                <div class="sr-map-legend">
                    <div class="sr-legend-item"><span class="sr-legend-dot sr-legend-dot--avail"></span>Disponibile</div>
                    <div class="sr-legend-item"><span class="sr-legend-dot sr-legend-dot--occ"></span>Occupata</div>
                </div>
            </div>
            ...
            <?php if (empty($roomSpots)): ?>
                <div class="sr-empty"><span></span><p>Nessuna sala riunione disponibile</p></div>
            <?php else: ?>
        ...
        <div class="sr-panel-body">
            ...
            <div>
                ...
                <form method="POST" id="booking-form">
                    ...
                    <button type="submit" class="sr-submit-btn" id="submit-btn">Conferma Prenotazione</button>
                </form>
            </div>
    ...
    <div class="sr-bookings-strip">
        ...
        <?php if (empty($userBookings)): ?>
            <div class="sr-empty"><span></span><p>Nessuna prenotazione sala attiva</p></div>
        <?php else: ?>
...
const C = {
    bg:'#f0f2f5', wall:'#c8cdd6', wallDark:'#9aa0ae', floorBg:'#e8ebf0', floorStripe:'#dde1e8',
    corridor:'#d0d4dc', corridorLine:'#bbbfc9',
    avail:'rgba(34,197,94,0.18)', availBrd:'#16a34a', availFill:'#dcfce7',
    occ:'rgba(239,68,68,0.15)', occBrd:'#dc2626', occFill:'#fee2e2',
    sel:'rgba(91,79,207,0.18)', selBrd:'#5b4fcf', selFill:'#ede9fe',
    textDark:'#1e1e2e', textMid:'#4a5568', textLight:'#718096', textWhite:'#ffffff',
    floorLabelBg:'#2d3748', floorLabelTx:'#ffffff',
    ROOM_W:160, ROOM_H:110, ROOM_GAP:40, FLOOR_PAD_X:80, FLOOR_PAD_Y:28,
    LABEL_W:64, CORRIDOR_H:36, FLOOR_HDR:32,
};

function groupByFloor() {
    const floors = {};
    roomsData.forEach(r => { const p = r.piano || 1; if (!floors[p]) floors[p] = []; floors[p].push(r); });
    return floors;
}
...
function computeLayout(W) {
    ...
    for (const fp of floorKeys) {
        ...
        const usedW = rooms.length * C.ROOM_W + (rooms.length - 1) * C.ROOM_GAP;
        ...
        floorLayouts.push({ piano:fp, rooms, startX, roomY:totalH+C.FLOOR_HDR+C.FLOOR_PAD_Y+C.CORRIDOR_H, floorY:totalH, floorH });
        totalH += floorH + 12;
    }
    totalH += 16;
...
function draw() {
    ...
    if (canvas.height !== layout.totalH) canvas.height = Math.max(400, layout.totalH);
    const H = canvas.height;
    ctx.clearRect(0, 0, W, H);
    ctx.fillStyle = C.bg; ctx.fillRect(0, 0, W, H);
    drawBgTexture(W, H);
    hitboxes = [];
    for (const fl of layout.floorLayouts) drawFloor(fl, W);
}

function drawBgTexture(W, H) {
    ctx.save(); ctx.globalAlpha = 0.04; ctx.strokeStyle = '#94a3b8'; ctx.lineWidth = 0.5;
    const step = 28;
...
function drawFloor(fl, W) {
    ...
    ctx.fillStyle = C.floorBg; ctx.fill();
    ctx.strokeStyle = C.wall; ctx.lineWidth = 1.5; ctx.stroke();
    roundRect(ctx, 4, floorY, C.LABEL_W-8, floorH, 8);
    ctx.fillStyle = C.floorLabelBg; ctx.fill();
    ctx.fillStyle = C.floorLabelTx; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.font = 'bold 11px system-ui';
    ctx.fillText('PIANO', 4+(C.LABEL_W-8)/2, floorY+floorH/2-9);
    ctx.font = 'bold 22px system-ui';
    ctx.fillText(piano, 4+(C.LABEL_W-8)/2, floorY+floorH/2+12);
    ctx.fillStyle = C.wall; ctx.fillRect(C.LABEL_W+8, floorY, W-C.LABEL_W-16, C.FLOOR_HDR);
    ctx.fillStyle = C.wallDark; ctx.fillRect(C.LABEL_W+8, floorY, 4, C.FLOOR_HDR);
    ctx.fillStyle = C.textDark; ctx.font = 'bold 12px system-ui'; ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
    ctx.fillText(`Piano ${piano} — Sale Riunioni`, C.LABEL_W+22, floorY+C.FLOOR_HDR/2);
    ctx.textAlign = 'right'; ctx.fillStyle = C.textLight; ctx.font = '11px system-ui';
    ctx.fillText(`${rooms.length} sal${rooms.length>1?'e':'a'}`, W-28, floorY+C.FLOOR_HDR/2);
    const corrY = floorY + C.FLOOR_HDR;
    ctx.fillStyle = C.corridor; ctx.fillRect(C.LABEL_W+8, corrY, W-C.LABEL_W-16, C.CORRIDOR_H);
    const cy = corrY + C.CORRIDOR_H/2;
    ctx.setLineDash([14,10]); ctx.strokeStyle = C.corridorLine; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(C.LABEL_W+20, cy); ctx.lineTo(W-20, cy); ctx.stroke();
    ctx.setLineDash([]);
    rooms.forEach((room, i) => {
        const rx = startX + i*(C.ROOM_W+C.ROOM_GAP), ry = roomY;
        ...
        ctx.strokeStyle = C.wallDark; ctx.lineWidth = 2; ctx.setLineDash([4,4]);
        ctx.beginPath(); ctx.moveTo(doorX, corrY+C.CORRIDOR_H); ctx.lineTo(doorX, ry); ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle = C.wallDark;
        ctx.beginPath(); ctx.arc(doorX, ry+4, 4, 0, Math.PI*2); ctx.fill();
    });
}

function drawRoom(x, y, room, isSelected) {
    const isOcc = room.status.toLowerCase() === 'occupato';
    ctx.save(); ctx.shadowColor='rgba(0,0,0,0.12)'; ctx.shadowBlur=8; ctx.shadowOffsetY=3;
    ctx.fillStyle = isSelected ? C.selFill : (isOcc ? C.occFill : C.availFill);
    roundRect(ctx, x, y, C.ROOM_W, C.ROOM_H, 8); ctx.fill(); ctx.restore();
    ctx.lineWidth = isSelected ? 2.5 : 1.5;
    ctx.strokeStyle = isSelected ? C.selBrd : (isOcc ? C.occBrd : C.availBrd);
    roundRect(ctx, x, y, C.ROOM_W, C.ROOM_H, 8); ctx.stroke();
    ctx.fillStyle = isSelected ? C.selBrd : (isOcc ? C.occBrd : C.availBrd);
    ctx.beginPath(); ctx.roundRect(x, y, C.ROOM_W, 6, [8,8,0,0]); ctx.fill();
    const cx = x + C.ROOM_W/2;
    const circY = y + 30;
    ctx.fillStyle = isOcc ? '#ef4444' : '#22c55e';
    ctx.beginPath(); ctx.arc(cx, circY, 14, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = isOcc ? 'rgba(239,68,68,0.15)' : 'rgba(34,197,94,0.15)';
    ctx.beginPath(); ctx.arc(cx, circY, 19, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.font = 'bold 13px system-ui'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(isOcc ? '✕' : '✓', cx, circY);
    ctx.fillStyle = isSelected ? '#3730a3' : C.textDark; ctx.font = 'bold 12px system-ui';
    ctx.fillText(room.name, cx, y+62);
    ctx.fillStyle = isOcc ? '#dc2626' : '#16a34a'; ctx.font = '10px system-ui';
    ctx.fillText(isOcc ? 'Occupata' : 'Disponibile', cx, y+78);
    if (room.capacita && room.capacita !== '-') {
        ctx.fillStyle = C.textLight; ctx.font = '10px system-ui';
        ctx.fillText('👥 '+room.capacita+' posti', cx, y+94);
    }
    if (isSelected) {
        ctx.strokeStyle = 'rgba(91,79,207,0.25)'; ctx.lineWidth = 6;
        roundRect(ctx, x-2, y-2, C.ROOM_W+4, C.ROOM_H+4, 10); ctx.stroke();
    }
}
...
function resize() { canvas.width = Math.max(500, wrap.clientWidth || 700); draw(); }
...
canvas.addEventListener('click', function(e) {
    ...
    for (const hb of hitboxes) {
        if (mx>=hb.x && mx<=hb.x+hb.w && my>=hb.y && my<=hb.y+hb.h) { openRoomPanel(hb.room.id); return; }
    }
});
...
canvas.addEventListener('mousemove', function(e) {
    ...
    const sx = canvas.width/rect.width, sy = canvas.height/rect.height;
    const mx = (e.clientX-rect.left)*sx, my = (e.clientY-rect.top)*sy;
    canvas.style.cursor = hitboxes.some(hb => mx>=hb.x && mx<=hb.x+hb.w && my>=hb.y && my<=hb.y+hb.h) ? 'pointer' : 'default';
});
...
function openRoomPanel(id) {
    ...
    document.getElementById('panel-status-pill').innerHTML =
        `<span class="sr-status-pill ${isOcc?'sr-status--occ':'sr-status--avail'}">${isOcc?'Occupata':'Disponibile'}</span>`;
    const pianoLabel = room.piano ? `Piano ${room.piano}` : '–';
    document.getElementById('panel-info-grid').innerHTML = `
        <div class="sr-info-tile"><span class="sr-info-tile-label">Piano</span><span class="sr-info-tile-val">${pianoLabel}</span></div>
        <div class="sr-info-tile"><span class="sr-info-tile-label">Capacità</span><span class="sr-info-tile-val">${room.capacita!=='-'?room.capacita+' posti':'–'}</span></div>
        <div class="sr-info-tile"><span class="sr-info-tile-label">Attrezzatura</span><span class="sr-info-tile-val" style="font-size:11px">${room.attrezzatura}</span></div>
        <div class="sr-info-tile"><span class="sr-info-tile-label">Orario</span><span class="sr-info-tile-val" style="font-size:11px">${room.disponibilita}</span></div>`;
    ...
    document.getElementById('data-inizio').value = ''; document.getElementById('data-fine').value = '';
    ...
    document.getElementById('data-inizio').min = minVal; document.getElementById('data-fine').min = minVal;
...
function renderTimeline(roomId) {
    ...
    const rawSlots = (occupiedSlots[roomId] || []).map(s => ({ start: new Date(s.inizio), end: new Date(s.fine) }))
        .filter(s => s.end > dayStart && s.start < dayEnd).sort((a,b) => a.start - b.start);
    ...
    const nowMark = nowPct>=0 && nowPct<=100
        ? `<div style="position:absolute;top:-3px;left:${nowPct}%;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;pointer-events:none;z-index:2"><div style="width:2px;height:26px;background:#f59e0b;border-radius:1px"></div><span style="font-size:9px;font-weight:700;color:#f59e0b;margin-top:2px">ora</span></div>` : '';
    const labels = [9,11,13,15,17,19].map(h => `<span style="position:absolute;left:${((h-9)/10*100).toFixed(1)}%;transform:translateX(-50%);font-size:9px;color:var(--clr-text-3);font-weight:600">${String(h).padStart(2,'0')}:00</span>`).join('');
    ...
    return `<div style="position:relative;padding-bottom:20px"><div style="display:flex;height:20px;border-radius:4px;overflow:hidden;border:1px solid var(--clr-border)">${bars}</div>${nowMark}<div style="position:absolute;bottom:0;left:0;width:100%;height:16px">${labels}</div></div><div style="margin-top:8px">${summary}</div>`;
}
...
function getFreeWindows(roomId) {
    ...
    const occupied = (occupiedSlots[roomId]||[]).map(s=>({start:new Date(s.inizio),end:new Date(s.fine)})).sort((a,b)=>a.start-b.start);
    const windows = [];
...
function renderFreeSlots(roomId) {
    ...
    return windows.map(fw => {
        const durMs = fw.end-fw.start, durH=Math.floor(durMs/3600000), durM=Math.floor((durMs%3600000)/60000);
        ...
        return `<div onclick="prefillSlot('${toLocalISO(fw.start)}','${toLocalISO(fw.end)}')"
                     ...
                     style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;background:var(--clr-success-bg);border:1px solid #86efac;border-left:3px solid var(--clr-success);border-radius:var(--radius-md);cursor:pointer;margin-bottom:6px;transition:background .15s">
                    <div>
...
function updateDuration() {
    ...
    const showErr = msg => { preview.style.display='none'; error.style.display=''; error.innerHTML='⚠️ '+msg; btn.disabled=true; };
    if (sDate < new Date())    return showErr('Non puoi prenotare nel passato.');
    ...
    if (hasConflict(startVal, endVal, roomId)) return showErr('Periodo sovrapposto a una prenotazione esistente.<br><small>Scegli uno dei periodi liberi suggeriti sopra.</small>');
