...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'cancel_booking') {
        ...
        if ($id_pren > 0) {
            ...
            $stmt->bind_param("ii", $id_pren, $id_utente); $stmt->execute();
            ...
            if ($assetRow) {
                ...
                $stmt->bind_param("ii", $id_pren, $id_utente);
                if ($stmt->execute()) {
                    $stmt->close();
...
<link rel="stylesheet" href="../../admin/uffici-tipologia-a/uffici.css">
</head>
<body>
      ...
      <div class="uf-bookings-list">
        ...
        <div class="uf-booking-item">
          ...
          <form method="POST" style="margin-top:6px">
            ...
            <input type="hidden" name="id_prenotazione" value="<?= (int)$b['id_prenotazione'] ?>">
            <button type="submit" class="uf-cancel-btn"
                    onclick="return confirm('Annullare questa prenotazione?')">Annulla</button>
          </form>
        </div>
        <?php endforeach;?>
      </div>
